...
function L:RegisterCallback(event, callback, ...)
  ...
  if not self.eventFrame.callbacks then
    self.eventFrame.callbacks = {}
  end
  if not self.eventFrame.callbacks[event] then
    self.eventFrame.callbacks[event] = {}
  end
  self.eventFrame.callbacks[event][callback] = {...}
  self.eventFrame:RegisterEvent(event)
end

function L:createText(f, layer, fontsize, outline, justifyh, shadow)
  local text = f:CreateFontString(nil, layer)
  text:SetFont(G.font, fontsize, outline)
...
--rLib:CreateSlashCmd
function L:CreateSlashCmd(addonName, shortcut, frames, color)
